...
class TestDataCleaner(unittest.TestCase):
    # Oppretter data som vi kan bruke som eksempler 
    def setUp(self):
        self.data = [
    ...
    # Tester om dataene fra values blir hentet ut riktig
    # Sjekker derfor lengden på DataFramen, at kolonnen vår "component finnes" og at første rad har riktig navn.
...
class TestDataQualityChecker(unittest.TestCase):
    ...
    # Sjekker at den finner en manglende verdi for hver komponent. 
    def test_check_missing_values(self):
        missing = self.checker.check_missing_values()
        self.assertEqual(missing["NO2"], 1)
        self.assertEqual(missing["PM10"], 1)

    # Sjekker for høye verdier. 
    # Sjekker at den finner en for høy verdi i NO2. Dobbelsjekker så at denne verdien er høyere enn 400. 
...
class TestObservationProcessor(unittest.TestCase):
    def test_extract_observations_with_valid_data(self):
        # Oppretter et datasett som brukes som eksempel. 
        sample_data = [
            {
    ...
    def test_extract_observations_with_missing_data(self):
        ...
        processor = ObservationProcessor(sample_data)
        result = processor.extract_observations()
        # Sjekker at den ikke finner noen observasjoner
